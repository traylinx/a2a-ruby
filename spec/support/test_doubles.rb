...
module A2ATestDoubles
  ...
  def mock_a2a_client(**options)
    ...
    allow(client).to receive(:get_task) do |task_id, **opts|
      build_task(id: task_id, state: options[:task_state] || "completed")
    end
    
    allow(client).to receive(:cancel_task) do |task_id, **opts|
      build_task(id: task_id, state: "canceled")
    end
    
    allow(client).to receive(:get_card) do |**opts|
      build_agent_card(options[:agent_card] || {})
    end
    
    allow(client).to receive(:resubscribe) do |task_id, **opts|
      create_test_sse_stream(
    ...
    allow(client).to receive(:set_task_push_notification_config) do |task_id, config, **opts|
      { id: test_uuid, taskId: task_id, **config }
    end
    
    allow(client).to receive(:get_task_push_notification_config) do |task_id, config_id, **opts|
      build(:push_notification_config, id: config_id)
    end
    
    allow(client).to receive(:list_task_push_notification_configs) do |task_id, **opts|
      [build(:push_notification_config)]
    end
    
    allow(client).to receive(:delete_task_push_notification_config) do |task_id, config_id, **opts|
      true
    end
  ...
  def mock_storage_backend(**options)
    ...
    allow(storage).to receive(:list_tasks) do |**filters|
      ...
      if filters[:state]
        tasks = tasks.select { |task| task.dig(:status, :state) == filters[:state] }
      end
      
      if filters[:context_id]
        tasks = tasks.select { |task| task[:context_id] == filters[:context_id] }
      end
  ...
  def mock_http_client(**options)
    ...
    allow(http_client).to receive(:post) do |path, body, headers|
    ...
    allow(http_client).to receive(:get) do |path, params, headers|
      if path.include?("agent-card")
        response_body = build_agent_card
      else
        response_body = options[:get_responses]&.dig(path) || { message: "Mock GET response" }
      end
  ...
  def mock_auth_strategy(**options)
    ...
    allow(auth).to receive(:authenticate) do |request|
      if options[:should_fail]
        raise A2A::Errors::AuthenticationError.new("Authentication failed")
      end
  ...
  def mock_grpc_client(**options)
    return nil unless defined?(GRPC)
    
    grpc_client = instance_double("GRPCClient")
    
    allow(grpc_client).to receive(:send_message) do |request|
      # Mock gRPC response
  ...
  def mock_push_notification_sender(**options)
    ...
    allow(sender).to receive(:send_notification) do |config, event|
      if options[:should_fail]
        raise A2A::Errors::ClientError.new("Failed to send notification")
      end
      
      { success: true, config_id: config[:id], event_type: event.class.name }
    end
    
    allow(sender).to receive(:register_config) do |task_id, config|
      config[:id] ||= test_uuid
      config
    end
    
    allow(sender).to receive(:unregister_config) do |task_id, config_id|
      true
    end
  ...
  def mock_rails_controller(**options)
    ...
    allow(controller).to receive(:request) do
      ...
      allow(request).to receive(:body).and_return(
        StringIO.new(options[:request_body] || build_json_rpc_request("test/method").to_json)
      )
      allow(request).to receive(:headers).and_return(options[:headers] || {})
      allow(request).to receive(:path).and_return(options[:path] || "/a2a/rpc")
      request
    end
