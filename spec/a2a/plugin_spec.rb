...
class TestAuthPlugin < A2A::Plugin::AuthPlugin
  ...
  def authenticate_request(request, **_options)
    request[:headers] ||= {}
...
RSpec.describe A2A::Plugin do
  ...
  describe ".register" do
    ...
    it "raises error for invalid plugin class" do
      ...
      end.to raise_error(A2A::Errors::PluginError, /must include A2A::Plugin::Base/)
    end
  end
  ...
  describe ".load" do
    ...
    it "raises error for unregistered plugin" do
      ...
      end.to raise_error(A2A::Errors::PluginError, /Plugin not found/)
    end
  end
  ...
  describe "hooks" do
    ...
    it "respects hook priority" do
      ...
      described_class.add_hook(:test_event, priority: 100) do |_data|
        results << "low_priority"
      end

      described_class.add_hook(:test_event, priority: 10) do |_data|
        results << "high_priority"
      end

      described_class.execute_hooks(:test_event, "data")

      expect(results).to eq(%w[high_priority low_priority])
    end
  end
end
...
RSpec.describe A2A::Plugin::Base do
  ...
  it "defines dependencies" do
    expect(plugin_class.dependencies).to eq(%i[dependency1 dependency2])
  end
...
RSpec.describe A2A do
  ...
  describe ".load_plugin" do
    ...
    it "loads a plugin" do
      plugin = described_class.load_plugin(:test_transport)
      expect(plugin).to be_a(TestTransportPlugin)
    end
  end
