...
RSpec.describe A2A::Configuration do
  ...
  describe "#load_from_file" do
    ...
    it "raises error for missing file" do
      ...
      end.to raise_error(A2A::Errors::ConfigurationError, /Configuration file not found/)
    end
  end
  ...
  describe "#validate!" do
    ...
    it "raises error for invalid timeout" do
      ...
      expect do
        config.validate!
      end.to raise_error(A2A::Errors::ConfigurationError, /default_timeout must be positive/)
    end
    ...
    it "raises error for blank protocol version" do
      ...
      expect do
        config.validate!
      end.to raise_error(A2A::Errors::ConfigurationError, /protocol_version cannot be blank/)
    end
    ...
    it "raises error for invalid transport" do
      ...
      expect do
        config.validate!
      end.to raise_error(A2A::Errors::ConfigurationError, /default_transport must be one of/)
    end
    ...
    it "raises error for invalid log level" do
      ...
      expect do
        config.validate!
      end.to raise_error(A2A::Errors::ConfigurationError, /log_level must be one of/)
    end
  end
end
...
RSpec.describe A2A do
  describe ".configure" do
    ...
    it "yields configuration for block configuration" do
      ...
      expect(described_class.config.default_timeout).to eq(45)
      expect(described_class.config.log_level).to eq(:debug)
    end
    ...
    it "validates configuration after setup" do
      ...
      end.to raise_error(A2A::Errors::ConfigurationError)
    end
  end
  ...
  describe ".configure_from_file" do
    ...
    it "configures from YAML file" do
      Tempfile.create(["a2a_config", ".yml"]) do |file|
        ...
        described_class.configure_from_file(file.path, environment: "test")
        expect(described_class.config.default_timeout).to eq(25)
        expect(described_class.config.log_level).to eq(:warn)
      end
    end
  end
  ...
  describe ".child_config" do
    ...
    it "creates child configuration" do
      described_class.configure { |c| c.default_timeout = 60 }
      child = described_class.child_config(log_level: :debug)
