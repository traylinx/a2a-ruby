...
#
RSpec.describe "A2A Memory Profiling", :performance, :memory do
  ...
  describe "Memory Leak Detection" do
    ...
    context "object lifecycle" do
      it "ensures proper object cleanup" do
        ...
        expect(total_increase).to be < 10000 # Allow some object growth but not excessive
      end
    end
  end
      ...
      it "analyzes garbage collection frequency" do
        ...
        1000.times do |i|
          ...
          request = build_json_rpc_request("gc_test_#{i}", {
            ...
            hash_data: (1..50).map { |j| ["key_#{j}", "value_#{j}"] }.to_h
          })
