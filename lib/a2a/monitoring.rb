    ...
    class MetricsCollector
      ...
      def setup_backends
        ...
        if defined?(Prometheus)
          add_backend(PrometheusBackend.new)
        end
        ...
        A2A::Monitoring.time('a2a_request_duration', **labels) do
          ...
          A2A::Monitoring.logger.with_correlation_id(correlation_id) do
            ...
            begin
              ...
              A2A::Monitoring.increment_counter('a2a_requests_error', **labels.merge(error_type: error.class.name))
              A2A::Monitoring.log(:error, "A2A request failed", method: method, request_id: correlation_id, error: error.message)
              raise
            end
          end
        end
        ...
        A2A::Monitoring.time('a2a_task_operation_duration', **labels) do
          A2A::Monitoring.logger.with_correlation_id(task_id) do
            ...
            begin
              ...
              A2A::Monitoring.increment_counter('a2a_task_operations_error', **labels.merge(error_type: error.class.name))
              A2A::Monitoring.log(:error, "Task operation failed", task_id: task_id, operation: operation, error: error.message)
              raise
            end
          end
        end
