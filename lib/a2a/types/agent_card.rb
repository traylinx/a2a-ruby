  ...
  module Types
    ...
    class AgentCard < BaseModel
      ...
      def initialize(name:, description:, version:, url:, preferred_transport:, skills:, 
        ...
        @additional_interfaces = additional_interfaces&.map { |i| i.is_a?(AgentInterface) ? i : AgentInterface.from_h(i) }
        @security = security
      ...
      def process_security_schemes(schemes)
        ...
        return schemes if schemes.is_a?(Hash) && schemes.values.all? { |v| v.is_a?(SecurityScheme) }
      ...
      def validate!
        ...
        validate_type(:supports_authenticated_extended_card, [TrueClass, FalseClass]) if @supports_authenticated_extended_card
      ...
      def validate_security_schemes
        ...
        @security_schemes.each do |name, scheme|
          unless scheme.is_a?(SecurityScheme)
            raise ArgumentError, "security_schemes[#{name}] must be a SecurityScheme"
          end
        end
      end
    ...
    class AgentInterface < BaseModel
      ...
      def validate_url_format
        begin
          uri = URI.parse(@url)
          unless uri.is_a?(URI::HTTP) || uri.is_a?(URI::HTTPS)
            raise ArgumentError, "url must be a valid HTTP or HTTPS URL"
          end
        rescue URI::InvalidURIError
    ...
    class AgentCardSignature < BaseModel
      ...
      def validate_protected_header_content
        ...
        unless header.is_a?(Hash) && header['alg']
          raise ArgumentError, "protected_header must contain a valid algorithm"
        end
      rescue => e
