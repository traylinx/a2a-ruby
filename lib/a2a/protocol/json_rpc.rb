...
begin
  require 'oj'
  JSON_PARSER = Oj
  JSON_PARSE_OPTIONS = { mode: :strict, symbol_keys: false }
  JSON_GENERATE_OPTIONS = { mode: :compat }
rescue LoadError
  JSON_PARSER = JSON
  JSON_PARSE_OPTIONS = {}
  JSON_GENERATE_OPTIONS = {}
end
    ...
    class JsonRpc
      ...
      def self.parse_request(json_string)
        ...
        begin
          ...
          raise A2A::Errors::ParseError.new("Invalid JSON: #{e.message}")
        end

        if parsed.is_a?(Array)
          # Batch request
          raise A2A::Errors::InvalidRequest.new("Empty batch request") if parsed.empty?
      ...
      def self.build_response(result: nil, error: nil, id:)
      ...
      def self.build_error_response(code:, message:, data: nil, id:)
      ...
      private_class_method def self.parse_single_request(hash)
        unless valid_request?(hash)
          raise A2A::Errors::InvalidRequest.new("Invalid request format")
        end
