    ...
    class Capability
      ...
      def validate_array_of_strings(value, field_name)
        unless value.is_a?(Array)
          raise ArgumentError, "#{field_name} must be an Array"
        end
        
        value.each_with_index do |item, index|
          unless item.is_a?(String)
            raise ArgumentError, "#{field_name}[#{index}] must be a String"
          end
        end
      end
      ...
      def validate_examples
        unless @examples.is_a?(Array)
          raise ArgumentError, "examples must be an Array"
        end
        
        @examples.each_with_index do |example, index|
          unless example.is_a?(Hash)
            raise ArgumentError, "examples[#{index}] must be a Hash"
          end
      ...
      def validate_object(data, schema, context)
        unless data.is_a?(Hash)
          raise ArgumentError, "#{context} must be an object"
        end
        
        # Check required properties
        required = schema[:required] || schema['required'] || []
        required.each do |prop|
          unless data.key?(prop) || data.key?(prop.to_sym)
            raise ArgumentError, "#{context} missing required property: #{prop}"
          end
        end
        ...
        if properties
          properties.each do |prop_name, prop_schema|
            ...
            if prop_value
              validate_against_schema(prop_value, prop_schema, "#{context}.#{prop_name}")
            end
          end
        end
      ...
      def validate_array(data, schema, context)
        unless data.is_a?(Array)
          raise ArgumentError, "#{context} must be an array"
        end
      ...
      def validate_string(data, context)
        unless data.is_a?(String)
          raise ArgumentError, "#{context} must be a string"
        end
        true
      end
      ...
      def validate_number(data, context)
        unless data.is_a?(Numeric)
          raise ArgumentError, "#{context} must be a number"
        end
        true
      end
      ...
      def validate_boolean_value(data, context)
        unless data.is_a?(TrueClass) || data.is_a?(FalseClass)
          raise ArgumentError, "#{context} must be a boolean"
        end
        true
      end
    end
    ...
    class CapabilityRegistry
      ...
      def register(capability)
        unless capability.is_a?(Capability)
          raise ArgumentError, "capability must be a Capability instance"
        end
