      ...
      class ApiKey
        ...
        def apply_to_request(request)
          ...
          when 'cookie'
            ...
            if existing_cookies
              request.headers['Cookie'] = "#{existing_cookies}; #{cookie_value}"
            else
        ...
        def valid?
          !@key.nil? && !@key.empty? && @key.is_a?(String)
        end
        ...
        def masked_key
          return '[empty]' if @key.nil? || @key.empty?
          return @key if @key.length <= 8
          
          "#{@key[0..3]}#{'*' * (@key.length - 8)}#{@key[-4..-1]}"
        end
        ...
        def inspect
          "#<A2A::Client::Auth::ApiKey:0x#{object_id.to_s(16)} #{to_s}>"
        end
        ...
        def validate_configuration!
          raise ArgumentError, "API key cannot be nil or empty" if @key.nil? || @key.empty?
