      ...
      class OAuth2
        ...
        def refresh_token
          ...
          unless response.success?
            raise A2A::Errors::AuthenticationError.new(
              "OAuth2 token request failed: #{response.status} - #{response.body}"
            )
          end
          ...
          unless token_data['access_token']
            raise A2A::Errors::AuthenticationError.new(
              "OAuth2 response missing access_token: #{token_data}"
            )
          end
          ...
          raise A2A::Errors::AuthenticationError.new(
            "OAuth2 token request failed: #{e.message}"
          )
        end
      end
    end
