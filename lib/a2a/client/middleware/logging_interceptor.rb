      ...
      class LoggingInterceptor < Base
        ...
        def call(request, context, next_middleware)
          ...
          begin
            ...
            duration = Time.now - start_time
            log_error(error, context, duration) if @log_errors
            raise error
          end
        end
        ...
        def mask_body(body)
          ...
          %w[password secret token key credential].each do |field|
            masked.each do |k, v|
              if k.to_s.downcase.include?(field)
        ...
        def sanitize_context(context)
          context.reject { |k, v| k.to_s.include?('password') || k.to_s.include?('secret') }
        end
