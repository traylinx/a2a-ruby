...
module A2A
  module Client
    module Middleware
      ...
      #
      class CircuitBreakerInterceptor < Base
        ...
        def execute_request(request, context, next_middleware)
          begin
            ...
            if circuit_breaker_error?(error)
              on_failure
            end
            raise error
          end
        ...
        def check_timeout_and_execute(request, context, next_middleware)
          if timeout_expired?
            ...
            raise A2A::Errors::AgentUnavailable.new(
        ...
        def validate_configuration!
          if @failure_threshold <= 0
            raise ArgumentError, "failure_threshold must be positive"
          end
          ...
          unless @expected_errors.is_a?(Array)
