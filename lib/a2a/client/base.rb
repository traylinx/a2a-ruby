    ...
    class Base
      ...
      def negotiate_transport(agent_card)
        ...
        if @config.use_client_preference?
          ...
          if agent_supports_transport?(agent_card, preferred)
            return preferred
          end
        end
        ...
        if @config.supports_transport?(agent_preferred)
          return agent_preferred
        end
      ...
      def get_endpoint_url(agent_card, transport)
        ...
        if agent_card.preferred_transport == transport
      ...
      def execute_with_middleware(request, context = {})
        # Create a chain of middleware calls
        chain = @middleware.reverse.reduce(proc { |req, ctx| yield(req, ctx) }) do |next_call, middleware|
