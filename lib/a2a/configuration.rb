...
module A2A
  ...
  class Configuration
    ...
    def load_from_environment
      ...
      @webhook_authentication_required = env_bool('A2A_WEBHOOK_AUTHENTICATION_REQUIRED', @webhook_authentication_required)
      @user_agent = env_string('A2A_USER_AGENT', @user_agent)

      # Redis configuration from environment
      redis_url = ENV['REDIS_URL'] || ENV['A2A_REDIS_URL']
      if redis_url
    ...
    def load_from_file(file_path, environment: nil)
      ...
      unless File.exist?(file_path)
        raise A2A::Errors::ConfigurationError, "Configuration file not found: #{file_path}"
      end
    ...
    def validate_basic_config
      ...
      raise A2A::Errors::ConfigurationError, "protocol_version cannot be blank" if protocol_version.nil? || protocol_version.empty?
      
      unless %i[debug info warn error fatal].include?(log_level)
    ...
    def validate_transport_config
      ...
      unless default_input_modes.is_a?(Array) && default_input_modes.all? { |mode| mode.is_a?(String) }
        raise A2A::Errors::ConfigurationError, "default_input_modes must be an array of strings"
      end

      unless default_output_modes.is_a?(Array) && default_output_modes.all? { |mode| mode.is_a?(String) }
    ...
    def validate_rails_config
      ...
      if defined?(Rails) && Rails.version < "6.0"
        raise A2A::Errors::ConfigurationError, "Rails integration requires Rails 6.0 or higher. Current version: #{Rails.version}"
      end
    end

    def validate_redis_config
      if redis_config && !redis_config.is_a?(Hash)
        raise A2A::Errors::ConfigurationError, "redis_config must be a hash"
      end

      if redis_config && redis_config[:url] && !redis_config[:url].is_a?(String)
