    ...
    class Handler
      ...
      def route_request(request, context: nil)
        ...
        unless @agent.class.a2a_method_registered?(request.method)
          raise A2A::Errors::MethodNotFound.new("Method '#{request.method}' not found")
        end
        
        # Get method definition for validation
        method_def = @agent.class.a2a_method_definition(request.method)
      ...
      def validate_agent!
        unless @agent.class.included_modules.include?(A2A::Server::Agent)
      ...
      def validate_request(request)
        unless request.is_a?(A2A::Protocol::Request)
          raise A2A::Errors::InvalidRequest.new("Invalid request object")
        end
        
        if request.method.nil? || request.method.empty?
          raise A2A::Errors::InvalidRequest.new("Method name is required")
        end
        
        unless request.params.is_a?(Hash) || request.params.is_a?(Array)
          raise A2A::Errors::InvalidParams.new("Parameters must be an object or array")
        end
      end
      ...
      def validate_method_parameters(method_name, params)
        ...
        begin
          ...
          raise A2A::Errors::InvalidParams.new("Parameter validation failed: #{e.message}")
        end
      end
