    ...
    module Agent
      ...
      module ClassMethods
        ...
        def a2a_method(name, **options, &block)
          raise ArgumentError, "Method name is required" if name.nil? || name.empty?
        ...
        def a2a_capability(name, &block)
          raise ArgumentError, "Capability name is required" if name.nil? || name.empty?
      ...
      def handle_a2a_request(request, context: nil)
        ...
        unless method_def
          raise A2A::Errors::MethodNotFound.new("Method '#{method_name}' not found")
        end
        ...
        unless request.notification?
          A2A::Protocol::JsonRpc.build_response(result: result, id: request.id)
        end
        ...
        unless satisfied
          if context.authenticated?
            raise A2A::Errors::AuthorizationFailed.new(
              "Method requires one of: #{security_requirements.join(', ')}"
            )
          else
      ...
      def build_middleware_chain(method_def, context)
        ...
        chain = ->(params, &block) { block.call }
        
        all_middleware.reverse.each do |middleware_def|
