  ...
  module Server
    module Middleware
      ...
      class LoggingMiddleware
        attr_reader :logger, :level, :format, :options

        # Logging formats
        FORMATS = [:simple, :detailed, :structured, :json].freeze
        ...
        def initialize(logger: nil, level: :info, format: :detailed, **options)
          ...
          @options = {
            ...
            log_response: false,
            log_errors: true,
            filtered_params: ['password', 'token', 'api_key', 'secret'],
        ...
        def log_detailed_start(request, context, request_id)
          ...
          if @options[:include_context] && context.authenticated?
        ...
        def log_detailed_success(request, context, result, duration, request_id)
        ...
        def log_detailed_error(request, context, error, duration, request_id)
        ...
        def log_structured_start(request, context, request_id)
          ...
          if @options[:log_params] && !request.params.empty?
            data[:params] = filter_sensitive_data(request.params)
          end
        ...
        def log_structured_success(request, context, result, duration, request_id)
          data = {
          ...
          if @options[:log_response] && result
            data[:response] = filter_sensitive_data(result)
          end
        ...
        def log_structured_error(request, context, error, duration, request_id)
          data = {
        ...
        def filter_sensitive_data(data)
          ...
          when Hash
            ...
            data.each do |key, value|
              if @options[:filtered_params].include?(key.to_s.downcase)
