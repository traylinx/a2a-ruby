  ...
  module Server
    ...
    class TaskManager
      ...
      def list_tasks_by_context(context_id)
        @storage.list_tasks_by_context(context_id)
      end
      ...
      def validate_state_transition(current_state, new_state)
        ...
        if terminal_states.include?(current_state)
          raise ArgumentError, "Cannot transition from terminal state '#{current_state}'"
        end
      ...
      def record_processing_time(duration)
        @metrics_mutex.synchronize do
          ...
          if total_ops > 0
            ...
            @performance_metrics[:avg_processing_time] = duration
          end
        end
      end
