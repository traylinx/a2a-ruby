      ...
      class Redis < Base
        ...
        def initialize(redis: nil, url: nil, namespace: 'a2a', ttl: nil)
          unless defined?(::Redis)
            raise LoadError, "Redis gem is required for Redis storage. Add 'redis' to your Gemfile."
          end
        ...
        def clear_all_tasks
          ...
          tasks = task_keys.map { |key| @redis.get(key) }.compact.map { |data| deserialize_task(data) }
