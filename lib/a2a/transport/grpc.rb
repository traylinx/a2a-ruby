  ...
  module Transport
    ...
    class Grpc
      ...
      def initialize(endpoint, config = {})
        raise A2A::Errors::TransportError, "gRPC is not available. Install the 'grpc' gem to use gRPC transport." unless GRPC_AVAILABLE
      ...
      def connect
        ...
        begin
          ...
          @stub.class.rpc_descs.keys.first&.tap do |method|
    ...
    class A2AServiceStub
      ...
      def send_message(request, options = {})
        raise A2A::Errors::CapabilityNotSupported, "gRPC service implementation not available"
      end

      def send_message_stream(request, options = {})
        raise A2A::Errors::CapabilityNotSupported, "gRPC service implementation not available"
      end

      def get_task(request, options = {})
        raise A2A::Errors::CapabilityNotSupported, "gRPC service implementation not available"
      end

      def cancel_task(request, options = {})
        raise A2A::Errors::CapabilityNotSupported, "gRPC service implementation not available"
      end

      def get_agent_card(request, options = {})
        raise A2A::Errors::CapabilityNotSupported, "gRPC service implementation not available"
      end
    end
  end
