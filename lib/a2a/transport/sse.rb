...
module A2A
  module Transport
    ...
    class SSE
      ...
      #
      def connect_with_callback(headers, &block)
        Thread.new do
          begin
      ...
      def establish_connection(headers, &block)
        ...
        response = http.get(headers: request_headers) do |req|
      ...
      def parse_sse_line(line)
        ...
        if line.start_with?('data: ')
          ...
          @config[:reconnect_delay] = line[7..-1].to_i
          nil
        else
          nil
        end
      ...
      def buffer_event(event)
        ...
        # Limit buffer size
        while @event_buffer.size > @config[:buffer_size]
          @event_buffer.shift
        end
      end
      ...
      #
      def start_heartbeat
        return unless @config[:heartbeat_interval] > 0
