    ...
    class DistributedTracing
      ...
      class << self
        ...
        def start_span(name, kind: SPAN_KIND_INTERNAL, parent: nil, **attributes)
          ...
          if @tracer && @tracer.respond_to?(:start_span)
      ...
      #
      class Span
        attr_reader :name, :kind, :trace_id, :span_id, :parent_span_id, :start_time, :end_time, :attributes, :events, :status
        ...
        def finish
          ...
          if DistributedTracing.current_span == self
            DistributedTracing.set_current_span(@parent)
          end
        ...
        def export_span
          ...
          if A2A.configuration.debug_tracing
            puts "Span: #{to_h.to_json}"
          end
        end
      end
