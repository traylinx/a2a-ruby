...
namespace :a2a do
  ...
  task validate: :environment do
    ...
    if ::Rails.version < "6.0"
      errors << "Rails version #{::Rails.version} is not supported. Minimum version is 6.0"
    end
    ...
    config = A2A.config
    if config.mount_path && !config.mount_path.start_with?("/")
      errors << "Mount path must start with '/'"
    end
    ...
    if agent_classes.empty?
      warnings << "No A2A agents found. Consider creating some with 'rails generate a2a:agent'"
    end
    ...
    agent_classes.each do |klass|
      ...
      if capabilities.empty?
        warnings << "#{klass.name} has no capabilities defined"
      end
      
      if methods.empty?
        warnings << "#{klass.name} has no A2A methods defined"
      end
    end
