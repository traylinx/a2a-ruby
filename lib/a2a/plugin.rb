    ...
    class << self
      ...
      def load(name, **config)
        ...
        begin
          ...
          if instance.respond_to?(:register_hooks)
      ...
      def unload(name)
        ...
        if instance.respond_to?(:cleanup)
          instance.cleanup
        end
      ...
      def loaded_plugins(type: nil)
        ...
        plugins = @loaded_plugins.map { |name| @registry[name][:instance] }.compact
...
module A2A
  module Errors
    ...
    #
    class PluginError < A2AError; end
  end
end